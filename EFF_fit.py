...
def EFF_Mr(r, gamma, dim=3):
    if r==0.0: return 0.0
    if dim==2:
        return 2*np.pi*(1. - (1 + r**2.)**(1-gamma/2))/(gamma-2)
    else:
...
#            nbins = 16
            rbins = np.logspace(logrmin,np.log10(np.percentile(r,95)),30) #np.log10(r50*10),20)
...
#            rbins = Rebin(r, rbins)
            ...
            a0 = r50/10
            rho0 =  mbin[mbin>0][0]/dvol[mbin>0][0] #m[r.argsort()][:10].sum() / (4*np.pi/3*np.sort(r)[10]**dim)
            p0 = (np.log(3./(4*np.pi)), np.log(1.), 6.)
            params, pcov = curve_fit(delta_M, rbins[1:], mbin, (np.log(rho0), np.log(a0), 2.5), maxfev=10**4, sigma=errors)
            rorder = r.argsort()
            Mr = m[rorder].cumsum()
